...
I think that *bike handling* in road cycling is a challenging and stimulating reaesarch avenue. However, there are at least two points of view, and mixing them up is not going to help, so let's be sure we got 'em first:
...
The trajectory that the riders follow while negotiating a corner are often called *racing lines* (a term inherited from motorsports) In the following figure, different phases of a corner are depicted. 
...
* Individuality: the 'optimal' strategy depends on the individual's engine. A good sprinter with a lot of power available might prefer a 'late apex' strategy, becasue he/she can start delivering the power sooner. 
* Environmental conditions:
  * Road conditions, with particular reference to the road surface and the resulting road-tire friction coefficient. In these regards, rain might be a game changer, especially in race with extended technical sections. A wet road of course is more slippery
  * Resistive forces
  * Slope
* Race conditions refer to the energy preservation strategies (i.e., the pacing strategies) that the riders are trying to implement. It is known that strong accelerations that require massive power outputs are mainly supported by the anaerobic alactacid metabolic pathway. These big bursts might not be ideal during long races, but they might be required during a breakaway. Again, higher average speeds and early apex stragies are advocated for those who don't like to sprint out after the corners. Indeed, corners are often used by the riders to briefly recover and get ready for the next most demanding race sections.  
