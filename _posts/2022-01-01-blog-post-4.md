...
I think that *bike handling* in road cycling is a challenging and fascinating research avenue. However, I noticed there are at least two points of view, and mixing them up is not going to help, so let's be sure we got 'em first:
...
As it often happens, the best riders can stand out from the group because their uncommon abilities. However, displaying uncommon *bike handling* abilities can be very dangerous. The limits of the bike-rider performance are dictated by physics, which places rigid constraints to the kind of accelerations (and decelerations) that the bike can sustain without crashing. High speeds are never dangerous *per se*, but they can easily result in unsustainable accelerations. Uncommon *bike handling* abilities therefore require uncommon willing to get closer and closer to the edge of crashing. 

In road cycling, this is particularly relevant while braking and while cornering. Both braking and cornering require the combination of: pulling the brake levers, steering, and changing body position. While braking can lead to strong longitudinal decelerations, cornering can lead to strong lateral accelerations. Forces needed to complete these actions are generated at the tire-road interface, and therefore depend upon the tire-road friction: the higher the friction, the higher the accelerations that a bike can sustain. 

It is well know by experience, that it's easy to experience high-speeds while going downhill. In the next figure, the sequence of actions required during corner negotiation are represented. 
...
<p align="center">
<img src="../images/adherence_ellipse.png" alt="adherence_ellipse" width="450"/>
</p>
...
The trajectory that the riders follow while negotiating a corner are often called *racing lines* (a term inherited from motor sports) In the following figure, different phases of a corner are depicted. 

With reasonable level of accuracy, we can say that cyclists mainly have two cornering strategies, which lead to pretty different trajectories (or *racing lines*, in the world of motor sports). Before getting to them, we need to define an important term, the *apex*, i.e.: 'the point on the inside portion of a corner that a rider passes closest to'. 
...
The 'late apex' strategy is typically characterized by a hard braking action somehow separated from the cornering action (remember our 'black line rider' of the previous sections?). To clip the 'late apex' 
...
* 🔋 Race conditions refer to the energy preservation strategies (i.e., the pacing strategies) that the riders are trying to implement. It is known that strong accelerations that require massive power outputs are mainly supported by the anaerobic alactic metabolic pathway. These big bursts might not be ideal during long races, but they might be required during a breakaway. Again, higher average speeds and early apex strategy is advocated for those who don't like to sprint out after the corners. Indeed, corners are often used by the riders to briefly recover and get ready for the next most demanding race sections.  

Road geometry, with particular reference to the space available on the road. 
