...
Whether this tool can be used to assess an athlete's "fatiguability" is an open question. What the preliminary results are suggesting together with the last available literature is that if an athlete is exhausted with WR>0%, it means that they are likley "fatigued" (or impaired by other unfavourable conditions such as heat or injury), and they could not replicate their best maximum mean power for a specific time-domain. The great advantages of WR, is that: 1) this can be assessed in real-time, and 2) the specific time/intensity domain that leads to exhausiton can be revealed. 

All-in-all the WR holds some promise and, in my view, it aligns well with the rising concepts of "durability" and "moving thresholds". Indeed, for WR, there are no thresholds at all, just upper limits made to be broken. 

# Important disclaimer about "fatigue"
...
On this note, one may argue that the WR model is a fatigue-free model, and it's true. If we want to use the WR to predict or prescribe a training session or a pacing strategy, we should bear in mind that, with "fatigue" kicking in as exercise progresses, an athlete's ability to reach their historical maximum is impaired. The WR=0% therefore represents a limit, but again, it's not a *sine qua non* condition for exhaustion. Exhaustion can occur way before that point.
